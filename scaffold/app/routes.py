...
def redirect(token: str, request: Request, db: Session = Depends(get_db)):
    ...
    if token in redirect_cache:
        ...
        if cached_expires is not None and cached_expires < datetime.now():
            redirect_cache.pop(token)
    ...
    if mapping.exires_at is not None and mapping.exires_at < datetime.now():
        raise HTTPException(status_code - 410, detail = "Link expired")
